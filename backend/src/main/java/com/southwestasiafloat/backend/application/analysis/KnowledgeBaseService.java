...
public class KnowledgeBaseService {
    ...
    public List<KnowledgeDocument> retrieveRelevant(List<String> tags, List<String> keywords, int limit) {
        return retrieveRelevantWithScores(tags, keywords, limit).documents();
    }

    public KnowledgeRetrievalResult retrieveRelevantWithScores(List<String> tags, List<String> keywords, int limit) {
        ...
        List<KnowledgeRetrievalHit> ranked = getAllDocuments().stream()
                ...
                .map(candidate -> new KnowledgeRetrievalHit(candidate.document(), candidate.score(), false))
                .collect(Collectors.toList());

        if (!ranked.isEmpty()) {
            return new KnowledgeRetrievalResult(
                    List.copyOf(normalizedTags),
                    List.copyOf(normalizedKeywords),
                    effectiveLimit,
                    ranked
            );
        }

        List<KnowledgeRetrievalHit> fallback = getAllDocuments().stream()
                .filter(document -> document.getTags() != null)
                .filter(document -> document.getTags().stream()
                ...
                .limit(effectiveLimit)
                .map(document -> new KnowledgeRetrievalHit(document, 0.0d, true))
                .collect(Collectors.toList());
        return new KnowledgeRetrievalResult(
                List.copyOf(normalizedTags),
                List.copyOf(normalizedKeywords),
